...
_SINGLE_INSTANCE_PORT = 19872

def _check_single_instance():
    """Try to bind the single-instance port.
    Returns a bound server socket when we are the first instance.
    If the port is already taken, signal the existing instance to show itself
    ...
    try:
        ...
        srv.listen(5)
        return srv          # we are the first (and only) instance
    except OSError:
        # Another instance is already running — tell it to show itself
        try:
            cli = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            cli.settimeout(2)
            cli.connect(('127.0.0.1', _SINGLE_INSTANCE_PORT))
            cli.sendall(b'show')
            cli.close()
        except Exception:
            pass
...
_single_instance_server = _check_single_instance()
# ─────────────────────────────────────────────────────────────────────────────
...
try:
    if os.name == 'nt':
        if sys.__stdout__:
            os.system('title Daniele\'s Scheduler Console')
except Exception:
    pass
...
root = tk.Tk()
root.title(f"Daniele's Scheduler v{VER}")
...
def apply_dark_theme():
    ...
    style.configure(".", background=SURFACE, foreground=FG, font=(FONT, 10),
                    bordercolor=BORDER, troughcolor=SURFACE2, focuscolor=ACCENT)
    style.configure("TFrame", background=SURFACE)
    style.configure("TLabel", background=SURFACE, foreground=FG, font=(FONT, 10))
    style.configure("TButton", background=SURFACE2, foreground=FG, font=(FONT, 10),
                    bordercolor=BORDER, focusthickness=0, relief="flat", padding=(8, 5))
    style.map("TButton",
        background=[("active", ACCENT), ("pressed", ACCENT_H)],
        foreground=[("active", "#000000"), ("pressed", "#000000")],
        relief=[("pressed", "flat"), ("active", "flat")])
    style.configure("TEntry", fieldbackground=SURFACE2, foreground=FG,
                    insertcolor=FG, bordercolor=BORDER, font=(FONT, 10), relief="flat")
    style.map("TEntry",
    ...
    style.configure("TCombobox", fieldbackground=SURFACE2, background=SURFACE2,
                    ...
                    selectforeground="#000000", font=(FONT, 10))
    style.map("TCombobox",
    ...
    style.configure("TCheckbutton", background=SURFACE, foreground=FG,
                    indicatorcolor=SURFACE2, font=(FONT, 10))
    style.map("TCheckbutton",
        background=[("active", SURFACE)],
        indicatorcolor=[("selected", ACCENT), ("active", ACCENT_H)])
    style.configure("TNotebook", background=BG, bordercolor=BG, tabmargins=[2, 5, 2, 0])
    style.configure("TNotebook.Tab", background=SURFACE2, foreground=FG2,
                    font=(FONT, 10, "bold"), padding=[14, 7])
    style.map("TNotebook.Tab",
        background=[("selected", SURFACE), ("active", "#333333")],
    ...
    style.configure("Treeview", background=SURFACE, foreground=FG,
                    fieldbackground=SURFACE, bordercolor=BORDER, rowheight=26,
                    font=(FONT, 10))
    ...
    style.configure("TLabelframe", background=SURFACE, foreground=FG2, bordercolor=BORDER)
    style.configure("TLabelframe.Label", background=SURFACE, foreground=ACCENT,
                    font=(FONT, 10, "bold"))

apply_dark_theme()

# System Tray Background Logic
tray_icon_obj = None

def quit_window(icon, menu_item):
    icon.stop()
    root.destroy()
    os._exit(0)
...
def withdraw_window():
    root.withdraw()
    try:
        img = Image.open(bundle_path("icon.ico"))
    except Exception:
    ...
    tray_icon_obj = pystray.Icon("SpotifyScheduler", img, f"Daniele's Scheduler v{VER}", menu)
    tray_icon_obj.title = f"Daniele's Scheduler v{VER}"
...
def _single_instance_listener():
    """Background thread: wait for a 'show' signal from a second instance."""
    while True:
        try:
            conn, _ = _single_instance_server.accept()
            data = conn.recv(16)
            conn.close()
            if b'show' in data:
                root.after(0, _restore_from_tray)
        except Exception:
            break

threading.Thread(target=_single_instance_listener, daemon=True).start()
# ─────────────────────────────────────────────────────────────────────────────
...
def export_playlist():
    try:
        ...
        for idx, playlist in enumerate(playlists):
            ...
            if idx % 2 == 0:
                playlist_listbox.itemconfig(idx, background="#222222")  # subtle stripe
    ...
    try:
        ...
        def show_tracks_and_confirm():
            ...
            listbox = tk.Listbox(listbox_frame, width=65, height=15,
                bg=SURFACE2, fg=FG, selectbackground=ACCENT, selectforeground="#000000",
...
console_text = tk.Text(console_frame, wrap="word", height=20, width=100,
    font=(FONT, 10), bg=SURFACE, fg=FG, insertbackground=FG,
...
try:
    ...
    icon_label = tk.Label(info_frame, image=icon_photo, bg=SURFACE)
    icon_label.image = icon_photo 
    icon_label.pack(pady=10)
except Exception as e:
    print(f"Failed to load icon: {error(e)}")    

info_text = tk.Text(info_frame, wrap="word", height=10, width=70,
    font=(FONT, 12), bg=SURFACE, fg=FG, insertbackground=FG,
    selectbackground=ACCENT, selectforeground="#000000",
    relief="flat", borderwidth=0)
...
info_text.insert("insert", f"{_('Author')}: Daniele\n\n")
info_text.insert("insert", f"Se si blocca chiamate o scrivete a Daniele.\n")

info_text.tag_config("header", font=(FONT, 14, "bold"), justify="center", foreground=ACCENT)
...
def choose_device_name():
    ...
    try:
        ...
        custom_font = font.Font(family=FONT, size=10)
        ...
        for device in devices:
            ...
            if listbox_idx % 2 == 0:
                device_listbox.itemconfig(listbox_idx, background="#222222")
    ...
    def update_calendar(self):
        ...
        for day in range(1, num_days + 1):
            ...
            btn = tk.Button(
                ...
                relief="flat",
                bd=0,
                bg=SURFACE if current_date != today else "#1a3a2a",
                fg=FG2 if current_date != today else ACCENT,
            ...
            if current_date == self.selected_date:
                btn.config(bg=ACCENT, fg="#000000", font=(FONT, 10, "bold"))
            ...
            for day, button in self.day_buttons:
                ...
                if date_str in schedule_data and any(schedule_data[date_str].values()):
                    ...
                    if date_str == today_str:
                        continue  # skip today
                    button.config(bg="#1a3a2a", fg=ACCENT)  # day with schedule
...
canvas = tk.Canvas(entries_frame, bg=SURFACE, highlightthickness=0)
            ...
            for time_range, entry_data in sorted_entries:
                try:
                    ...
                    chk = tk.Checkbutton(entry_row, variable=rq_var, command=on_rq_change, width=2, anchor="center",
                        bg=SURFACE, fg=FG, activebackground=SURFACE, selectcolor=SURFACE2,
...
def edit_schedule_entry(time_range, current_playlist, current_playlist_name):
    try:
        ...
        for idx, playlist in enumerate(playlists):
            ...
            playlist_id_map[idx] = playlist['id']
            if idx % 2 == 0:
                edit_playlist_listbox.itemconfig(idx, background="#222222")
...
def cached_spotify_data(data_type, force=False):
    ...
    if (t.time() - cache[data_type]["timestamp"] < 15) and not force:
...
def main():
    ...
    print(f"# Daniele's Scheduler v{VER}")
    print("# Se non funziona chiedetemi e vi spiego come fare")
    ...
    def title_loop(lastdate=None):
        try:
            ...
            if lastdate!=now: #update title only when time changes
                    ...
                    try:
                        if not config['WEEKDAYS_ONLY'] or (config['WEEKDAYS_ONLY'] and datetime.today().weekday() < 5):
                            if empty_schedule:
                                ...
                                elif closest_start_time >= (datetime.now() - timedelta(seconds=10)).time():
                ...
                root.title(f"Daniele's Scheduler v{VER} | {now}{nextplay} {newupdate}")
        except Exception:
            pass
