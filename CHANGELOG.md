...
- **Premium UI**: New dark-themed, responsive dashboard with better performance and aesthetics.
- **Single Instance Check**: Prevent multiple application instances; launching the app while it's running will now just open the dashboard in your browser.
- **Mandatory Setup**: Added a mandatory setup modal to ensure Spotify credentials are correctly configured before the app starts.
- **System Tray Integration**: Added a system tray icon (Windows) for quick dashboard access and application management.
- **Improved Documentation**: Added `SETUP.txt` with clear instructions for the new web-based workflow.

### Changed:
- **Dependencies**: Updated `flask`, `packaging`, `requests`, and `Pillow` to their latest versions for improved security and performance.
- Improved overall stability and error handling for Spotify API interactions.
